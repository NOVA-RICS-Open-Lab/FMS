...
////////////////////////////////////////////////////////////////////////////////////////////////////
uint8_t DALI::receiveData(void)
{
	uint8_t receivedData = 0;
	
	for ( int8_t i = 7 ; i >= 0 ; i-- ) {
		delayMicroseconds(833);
		receivedData |= digitalRead(rxPin) << i;
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
int16_t DALI::waitResponse(void)
{
	// Check that the DALI node will answer
	uint16_t n;
	for (n = 0; n < 5000; n++) {
		if (digitalRead(rxPin) == 0) break;
		delayMicroseconds(1);
	}
	if (n == 5000) return -1;

	for (uint16_t n = 0 ; n < 50000 ; n++) {
...
/*
int8_t DALI::isPowerOn(uint8_t nodeNumber)
{
	sendCommand(((nodeNumber << 1) & 0x7E) | 0x01, QUERY_LAMP_POWER_ON);
	
	int16_t response = waitResponse();
	return (response == -1) ? -1 : (response == 0xFF) ? 0 : 1;
}
*/

int16_t DALI::getIntensityValue(uint8_t nodeNumber)
{
	sendCommand(((nodeNumber << 1) & 0x7E) | 0x01, QUERY_ACTUAL_LEVEL);
